...
class MockMonitor {
    constructor(mocks) {
        ...
        this.transformCallbacks = new Map();
    }
    recordTransformCallback(token, transform) {
    ...
    async registerResource(req, callback) {
        try {
            const transforms = req
                .getTransformsList()
                .map((cb) => this.transformCallbacks.get(cb.getToken()))
    ...
    registerStackTransform(req, callback) {
        const transform = this.transformCallbacks.get(req.getToken());
        if (transform !== undefined) {
            this.mocks.registerTransform?.(transform);
        }
        callback(null, {});
    }
    registerStackInvokeTransform(req, callback) {
        const transform = this.transformCallbacks.get(req.getToken());
        if (transform !== undefined) {
            this.mocks.registerInvokeTransform?.(transform);
        }
        callback(null, {});
    }
